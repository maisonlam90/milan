...
export function CollapsibleItem({ data }) {
  const { transKey, Icon, childs = [] } = data; // ← bỏ path
  const { t } = useTranslation();
  const { isRtl } = useLocaleContext();
  const ChevronIcon = isRtl ? ChevronLeftIcon : ChevronRightIcon;

  // Dropdown cục bộ, mặc định mở
  ...
  return (
    <div className="relative flex flex-1 flex-col px-3">
      ...
      {/* Panel con: luôn render; ẩn/hiện bằng CSS */}
      <div className={clsx("px-3 py-1.5 flex flex-col space-y-1", !open && "hidden")}>
        {childs.length > 0 ? (
          childs.map((child) => <MenuItem key={child.path || child.id} data={child} />)
        ) : (
          <div className="px-1 py-1 text-[11px] text-gray-400">Không có mục nào</div>
        )}
      </div>
    </div>
  );
